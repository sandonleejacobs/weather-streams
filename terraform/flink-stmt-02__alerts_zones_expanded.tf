resource "confluent_flink_statement" "create_active_alerts_zone_expanded" {
  organization {
  ...
  statement     = file("../flinksql/src/main/sql/create/02__create_ws_active_alerts_zone_expanded.sql")
  properties = {
  ...
  depends_on = [
    ...
    confluent_flink_statement.load_ws_active_alerts
  ]
...
resource "confluent_flink_statement" "load_ws_active_alerts_zone_expanded" {
  organization {
    id = data.confluent_organization.main.id
  }
  environment {
  ...
  statement     = file("../flinksql/src/main/sql/load/02__load_ws_active_alerts_zone_expanded.sql")
  properties = {
    "sql.current-catalog"  = confluent_environment.cc_env.display_name
    "sql.current-database" = confluent_kafka_cluster.basic.display_name
  }
