resource "confluent_flink_statement" "create_active_alerts_by_zone" {
  organization {
  ...
  statement     = file("../flinksql/src/main/sql/create-tables/ws_active_alerts_by_zone.sql")
  properties = {
...
resource "confluent_flink_statement" "insert_into_active_alerts_by_zone" {
  organization {
    id = data.confluent_organization.main.id
  }
  environment {
  ...
  statement     = file("../flinksql/src/main/sql/insert-into/ws_active_alerts_by_zone.sql")
  properties = {
    "sql.current-catalog"  = confluent_environment.cc_env.display_name
    "sql.current-database" = confluent_kafka_cluster.basic.display_name
  }
